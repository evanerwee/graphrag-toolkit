...
class SourceGraphBuilder(GraphBuilder):
    """Handles the logic for building and inserting source nodes into a graph
    database.
    ...
    def build(self, node:BaseNode, graph_client: GraphStore, **kwargs:Any):
        """
        Builds a node in the graph database, inserting a source if a valid source_id is
        provided in the node metadata. Processes metadata associated with the source,
        formats it appropriately, and updates the graph database with it. The method
        constructs and executes a Cypher query, ensuring proper assignment of properties
        both on creation and on updates of the source.

        :param node: A BaseNode instance representing a node in the graph, containing its
            metadata and other identifying attributes.
        :param graph_client: An instance of GraphStore that provides methods for
            executing queries and other related database operations.
        :param kwargs: Additional keyword arguments that may be used during processing.
        :return: None
        """
