...
class TopicNodeBuilder(NodeBuilder):
    """Builds Topic-related nodes by processing existing nodes and their
    metadata.
    ...
    def _add_chunk_id(self, node:TextNode, chunk_id:str):
        """Updates the given node's metadata to include a new chunk ID. This
        method retrieves the topic associated with the node, adds the given
        chunk ID to the list of chunk IDs (ensuring no duplicates), and updates
        the node's metadata with the modified topic information.
    ...
    def _add_statements(self, node:TextNode, statements:List[Statement]):
        """Adds a list of statements to the metadata of a given TextNode.
        Existing statements in the node metadata are preserved, and new
        statements are added if they are not configured to be ignored by the
        build filters.
    ...
    def build_nodes(self, nodes:List[BaseNode]):
        """Builds a list of topic nodes derived from the provided nodes. This
        function maps chunks and their contained relationships, topics, and
        statements into structured nodes. It ensures that unique topic
        identifiers are created for each source, and it manages the connections
        between source IDs and chunk IDs. The resulting nodes are prepared for
        further indexing operations.
