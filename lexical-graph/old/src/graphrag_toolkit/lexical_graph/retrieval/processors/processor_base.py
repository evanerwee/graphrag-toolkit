...
class ProcessorBase(object):
    ...
    def _log_results(
        self, retriever_name: str, title: str, search_results: SearchResultCollection
    ):
        """Logs debug information about intermediate search results if
        debugging is enabled for the current processor. The log contains the
        retriever name, processor name, title, and formatted search results.
        ...
        if processor_name in self.args.debug_results and logger.isEnabledFor(
            ...
            logger.debug(
                f'''Intermediate results [{retriever_name}.{processor_name}] {title}: {search_results.model_dump_json(
                indent=2, 
                exclude_unset=True, 
    ...
    def _apply_to_search_results(
        self,
        search_results: SearchResultCollection,
        search_result_handler: Callable[[int, SearchResult], SearchResult],
        **kwargs,
    ):
        """Applies a given handler to a collection of search results and
        processes them based on the result of the handler function. Each search
        result is passed to the handler function along with its index and any
        additional keyword arguments. The results returned by the handler are
        preserved only if they contain either topics or statements.
    ...
    def _apply_to_topics(
        self,
        search_result: SearchResult,
        topic_handler: Callable[[Topic], Topic],
        **kwargs,
    ):
        """Applies a handler function to all topics in a `search_result` and
        updates the result with only topics that satisfy specific criteria.
        Modifications to topics are determined by the provided `topic_handler`.
    ...
    def _format_statement_context(
        self, source_str: str, topic_str: str, statement_str: str
    ):
        """Formats a given statement in the context of a specified topic and
        source.
    ...
    def _log_counts(
        ...
        topic_count = sum(
            [len(search_result.topics) for search_result in search_results.results]
        )
        statement_count = sum(
            [
        ...
        logger.debug(
            f'[{retriever_name}.{type(self).__name__}] {title}: [results: {result_count}, topics: {topic_count}, statements: {statement_count}]'
        )
    ...
    def process_results(
        ...
        """Processes search results by applying specific modifications and
        logging before and after states.
