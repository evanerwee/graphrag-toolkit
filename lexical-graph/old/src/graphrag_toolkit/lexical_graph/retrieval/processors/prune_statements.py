...
class PruneStatements(ProcessorBase):
    """PruneStatements is responsible for processing search results by pruning
    statements below a specified score threshold.
    ...
    def __init__(self, args: ProcessorArgs, filter_config: FilterConfig):
        """Initializes the class instance and sets up the filter configuration
        and processing logic by leveraging the provided arguments.
    ...
    def _process_results(
        ...
        """Processes the search results by applying pruning operations based on
        a defined statement pruning threshold. Specifically, it filters out
        statements within topics that do not meet the required score threshold
        and applies this pruning to search results.
        ...
        def prune_statements(topic: Topic):
            """Processes the results from a search by filtering out statements
            based on a defined score threshold. This functionality allows only
            those statements that meet or exceed the threshold to be retained
            in the `Topic.statements`.
        ...
        def prune_search_result(index: int, search_result: SearchResult):
            """A processor class to prune search results by applying a specific
            pruning method to topics within each search result.
