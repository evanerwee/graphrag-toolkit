...
SourceInfoAccessorType = Union[
    str, List[str], Template, Callable[[Dict[str, Any]], str]
]
...
def source_info_template(
    ...
    """Generates a function for formatting source information using a provided
    template.
    ...
    def source_info_template_fn(source_properties: Dict[str, Any]) -> str:
        """Dynamically creates a string formatting function based on the
        provided template. This function is meant to substitute placeholder
        variables in the template with values from a dictionary of source
        properties.
...
def source_info_keys(keys: List[str]) -> Callable[[Dict[str, Any]], str]:
    ...
    def source_info_keys_fn(source_properties: Dict[str, Any]) -> str:
        """Generates a function that retrieves the value associated with the
        first matching key in a dictionary from a predefined list of keys.
...
class EnrichSourceDetails(BaseNodePostprocessor):
    ...
    def class_name(cls) -> str:
        """Determines and returns the name of the class to represent the source
        details.
    ...
    def _get_source_info(self, source_metadata, source) -> str:
        """Retrieves source information based on metadata and configured
        accessor.
        ...
        if isinstance(accessor, str):
            fn = (
                source_info_template(accessor)
                if '$' in accessor
                else source_info_keys([accessor])
            )
    ...
    def _postprocess_nodes(
        ...
        """Postprocesses a list of nodes to retrieve and update source
        information and search result details.
        ...
        for node in nodes:
            ...
            if source_metadata:
                source_info = self._get_source_info(
                    source_metadata, search_result.source.sourceId
                )
                search_result.source = str(source_info)
                node.node.text = search_result.model_dump_json(
