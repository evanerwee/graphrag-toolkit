...
class OpenSearchVectorIndexFactory(VectorIndexFactoryMethod):
    ...
    def try_create(
        ...
        """Attempts to create a list of vector indexes using the provided index
        names and vector index information. This method checks if a supported
        endpoint configuration is provided and uses it to initialize
        appropriate vector indexes. Raises ImportError if the required module
        is not available.
        ...
        elif vector_index_info.startswith('https://') and vector_index_info.endswith(
            OPENSEARCH_SERVERLESS_DNS
        ):
        ...
        if endpoint:
            try:
                ...
                logger.debug(
                    f'Opening OpenSearch vector indexes [index_names: {index_names}, endpoint: {endpoint}]'
                )
                return [
                    OpenSearchIndex.for_index(index_name, endpoint, **kwargs)
