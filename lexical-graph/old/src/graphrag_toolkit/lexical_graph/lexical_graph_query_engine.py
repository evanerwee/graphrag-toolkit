...
class LexicalGraphQueryEngine(BaseQueryEngine):
    ...
    def __init__(self,
        ...
        # --- Backward-compatible support for prompt_provider ---
        prompt_provider: Optional[PromptProvider] = kwargs.pop("prompt_provider", None)

        if prompt_provider:
            try:
                system_prompt = prompt_provider.get_system_prompt()
                user_prompt = prompt_provider.get_user_prompt()
                logger.info(f"[Prompt Debug] Using prompt provider: {prompt_provider.__class__.__name__}")
            except Exception as e:
                logger.exception("[Prompt Debug] Failed to fetch prompts from prompt_provider")
                system_prompt = ANSWER_QUESTION_SYSTEM_PROMPT
                user_prompt = ANSWER_QUESTION_USER_PROMPT
        else:
            system_prompt = ANSWER_QUESTION_SYSTEM_PROMPT
            user_prompt = ANSWER_QUESTION_USER_PROMPT
        # --- End prompt_provider support ---
