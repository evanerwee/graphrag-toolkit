...
SubRetrieverType = Union[
    ChunkBasedSearch, TopicBasedSearch, Type[ChunkBasedSearch], Type[TopicBasedSearch]
]


class EntityContextSearch(TraversalBasedBaseRetriever):
    """A retriever implementation designed to perform entity-context-based
    search within a graph database. It retrieves relevant nodes and their
    contexts based on the input query and employs advanced filtering and
    scoring mechanisms to refine results.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initializes an instance of the class with specified parameters. This
        constructor sets up graph and vector stores, optional processing
        arguments, a list of processors, a sub-retriever, and filter
        configuration to create a specific retrieval or searching mechanism.
    ...
    def _get_entity_contexts(self, start_node_ids: List[str]) -> List[str]:
        """Fetches and processes the context of entities based on relationships
        ...
        properties = {'entityIds': list(all_entity_ids)}
        ...
        for parent, children in entity_map.items():
            ...
            for child in children:
                ...
                if child_score <= (
                    self.args.ecs_max_score_factor * parent_score
                ) and child_score >= (self.args.ecs_min_score_factor * parent_score):
                ...
                scored_entity_contexts.append(
                    {
                        'entities': context_entities[
                            : self.args.ecs_max_entities_per_context
                        ],
                        'score': parent_score,
                    }
                )

        scored_entity_contexts = sorted(
            scored_entity_contexts, key=lambda ec: ec['score'], reverse=True
        )
        ...
        for scored_entity_context in scored_entity_contexts:
            ...
            all_entity_contexts.extend(
                [entities[x : x + 3] for x in range(0, max(1, len(entities) - 2))]
            )
    ...
    def _get_sub_retriever(self):
        """Retrieves or constructs a sub-retriever based on the type of the
        existing sub-retriever or initializes a new one with the provided
        configuration.
        ...
        sub_retriever = (
            self.sub_retriever
            if isinstance(self.sub_retriever, TraversalBasedBaseRetriever)
    ...
    def do_graph_search(
        ...
        """Executes a graph-based search query starting from provided node IDs
        ...
        for entity_context in entity_contexts:
            if entity_context:
                results = sub_retriever.retrieve(
                    QueryBundle(query_str=', '.join(entity_context))
                )
                for result in results:
        ...
        if retriever_name in self.args.debug_results and logger.isEnabledFor(
            ...
            logger.debug(
                f'''Entity context results: {search_results_collection.model_dump_json(
                    indent=2, 
                    exclude_unset=True, 
