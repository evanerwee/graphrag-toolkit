...
class VectorStore(BaseModel):
    ...
    indexes: Optional[Dict[str, VectorIndex]] = Field(
        description='Vector indexes', default_factory=dict
    )

    def get_index(self, index_name):
        """Retrieves the vector index associated with the given index name. If
        the specified index name is not recognized or has not been registered
        in the indexes dictionary, it returns a dummy index instead. The method
        ensures that only valid index names are processed and handled
        appropriately.
        ...
        if index_name not in ALL_EMBEDDING_INDEXES:
            raise ValueError(
                f'Invalid index name ({index_name}): must be one of {ALL_EMBEDDING_INDEXES}'
            )
