...
from graphrag_toolkit.lexical_graph.storage.vector.opensearch_vector_index_factory import (
...
from graphrag_toolkit.lexical_graph.storage.vector.pg_vector_index_factory import (
...
VectorIndexFactoryMethodType = Union[
    VectorIndexFactoryMethod, Type[VectorIndexFactoryMethod]
]

_vector_index_factories: Dict[str, VectorIndexFactoryMethod] = {
    c.__name__: c()
    for c in [
        OpenSearchVectorIndexFactory,
        PGVectorIndexFactory,
...
class VectorStoreFactory:
    """Manages the registration and creation of vector index factories and
    vector stores.
    ...
    def register(factory_type: VectorIndexFactoryMethodType):
        """Registers a factory method for vector index creation. This method
        allows the addition of custom factory methods to the global registry,
        enabling dynamic instantiation of vector index objects based on the
        factory type.
        ...
        if isinstance(factory_type, type):
            if not issubclass(factory_type, VectorIndexFactoryMethod):
                raise ValueError(
                    f'Invalid factory_type argument: {factory_type.__name__} must inherit from VectorIndexFactoryMethod.'
                )
            ...
            if not isinstance(factory_type, VectorIndexFactoryMethod):
                raise ValueError(
                    f'Invalid factory_type argument: {factory_type_name} must inherit from VectorIndexFactoryMethod.'
                )
    ...
    def for_vector_store(
        ...
        for factory in _vector_index_factories.values():
            vector_indexes = factory.try_create(
                index_names, vector_store_info, **kwargs
            )
            if vector_indexes:
                return VectorStore(indexes={i.index_name: i for i in vector_indexes})

        raise ValueError(
            f'Unrecognized vector store info: {vector_store_info}. Check that the vector store connection info is formatted correctly, and that an appropriate vector index factory method is registered with VectorStoreFactory.'
        )

    @staticmethod
    def for_composite(vector_store_list: List[VectorStore]):
        """Combines multiple VectorStore instances into a single instance by
