...
class YouTubeReaderProvider:
    ...
    def __init__(self, config: YouTubeReaderConfig):
        ...
        # Get proxy from config or fall back to GraphRAGConfig
        self.proxy_url = config.proxy_url or GraphRAGConfig.youtube_proxy_url
        logger.debug(f"Initialized YouTubeReaderProvider with language={config.language}, proxy={'configured' if self.proxy_url else 'none'}")
    ...
    def read(self, input_source: Union[str, List[str]]) -> List[Document]:
        ...
        if self.proxy_url:
            logger.info(f"Setting proxy for YouTube requests: {self.proxy_url.split('@')[-1] if '@' in self.proxy_url else self.proxy_url}")
