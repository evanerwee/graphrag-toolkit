...
class StreamingJSONLReaderConfig(ReaderProviderConfig):
    """Configuration for StreamingJSONLReaderProvider - memory-efficient JSONL processing."""
    batch_size: int = 100  # Documents per batch yield
    text_field: Optional[str] = "text"  # JSON key for document text, None = use entire line
    strict_mode: bool = False  # Raise on first malformed line vs skip
    log_interval: int = 10000  # Log progress every N lines
    metadata_fn: Optional[Callable[[str], Dict[str, Any]]] = None
...
class YouTubeReaderConfig(ReaderProviderConfig):
    language: str = "en"
    metadata_fn: Optional[Callable[[str], Dict[str, Any]]] = None
    # Proxy configuration for bypassing YouTube IP bans (e.g., from AWS)
