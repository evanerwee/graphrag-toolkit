...
class VectorIndexing(NodeHandler):
    ...
    def _has_valid_content(self, node: BaseNode) -> bool:
        """Check if a node has valid (non-empty) content for embedding.
        
        Args:
            node: The node to check.
            
        Returns:
            True if the node has non-empty, non-whitespace content; False otherwise.
        """
    ...
    def accept(self, nodes: List[BaseNode], **kwargs: Any):
        ...
        applied based on metadata. 
        
        Error handling is per-node: if a node fails to index, it is logged and skipped,
        allowing successfully processed nodes to be yielded and checkpointed. This ensures
        partial progress is preserved even when individual nodes fail.
        ...
        # Track failed nodes for summary logging
        failed_nodes = []
        skipped_empty_nodes = []
                ...
                if [key for key in [INDEX_KEY] if key in node.metadata]:
                    try:
                        ...
                        if index_name in ALL_EMBEDDING_INDEXES:
                            ...
                            if not self._has_valid_content(node):
                                logger.warning(
                                    f"Skipping node {node.node_id} with empty/whitespace content for index '{index_name}'"
                                )
                                skipped_empty_nodes.append(node.node_id)
                                # Still yield the node so it can be checkpointed as "processed"
                        ...
                        # This ensures partial progress is preserved via checkpoints
                        logger.error(
                            f"Failed to index node {node.node_id}: {type(e).__name__}: {e}. "
                            f"Skipping node and continuing with remaining nodes."
                        )
                        failed_nodes.append((node.node_id, str(e)))
                        # Don't yield failed nodes - they won't be checkpointed
        ...
        # Log summary of skipped/failed nodes
        if skipped_empty_nodes:
            logger.warning(
                f"Vector indexing complete. Skipped {len(skipped_empty_nodes)} nodes with empty content."
            )
        if failed_nodes:
            logger.error(
                f"Vector indexing complete with {len(failed_nodes)} failed nodes. "
                f"Failed node IDs: {[nid for nid, _ in failed_nodes[:10]]}{'...' if len(failed_nodes) > 10 else ''}"
            )
