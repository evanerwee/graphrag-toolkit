...
class OpenSearchIndex(VectorIndex):
    ...
    def add_embeddings(self, nodes):
        ...
        Nodes with empty or whitespace-only content are filtered out and logged
        as warnings to prevent embedding API failures.
        ...
        # Filter out nodes with empty/whitespace content before embedding
        valid_nodes = []
        for node in nodes:
            if node.node_id in nodes_to_embed_map:
                ...
                if content and content.strip():
                    ...
                    logger.warning(
                        f"[OpenSearchIndex] Skipping node {node.node_id} with empty/whitespace content"
                    )
        
        nodes_to_embed = valid_nodes
