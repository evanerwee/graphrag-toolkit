...
class LexicalGraphQueryEngine(BaseQueryEngine):
    ...
    def _clean_source_path(self, source: str) -> str:
        """
        Extract a clean, human-readable source name from a full source path.
        
        The source path may contain metadata like:
        "/tmp/tmp7fpeelcq/tagging-best-practices-2.pdf (wiz-issue-triage#SECURITYCONTEXT, ...)"
        
        We want to extract just: "tagging-best-practices-2.pdf"
        """
        if not source:
            return "Unknown Source"
        
        # If source contains parentheses with metadata, extract just the path before it
        if ' (' in source:
        ...
        # If filename is empty (e.g., source was just metadata), try to extract from the metadata
        if not filename or filename == source:
            # Try to find a filename pattern in the original source
            import re
            # Look for common file extensions
            match = re.search(r'([^/\\,\s]+\.(pdf|docx?|xlsx?|csv|txt|md|json|yaml|yml))', source, re.IGNORECASE)
            if match:
                filename = match.group(1)
            else:
                # Last resort: use first meaningful segment
                filename = source.split(',')[0].strip() if ',' in source else source
    ...
    def _format_as_text(self, json_results):
        ...
        for json_result in json_results:
            source = self._clean_source_path(json_result.get('source', ''))
