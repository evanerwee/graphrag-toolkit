---
title: Semantic-Guided Search (Deprecated)
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="danger" title="Deprecated">
  The semantic-guided retriever is deprecated and will be removed in a future release. Use [traversal-based search](/graphrag-toolkit/lexical-graph/traversal-based-search/) instead, which provides better performance and lower storage costs.
</Aside>

The recommended method for query and retrieval is to use the [traversal-based search](/graphrag-toolkit/lexical-graph/traversal-based-search/) operation. The lexical-graph does, however, also currently support semantic-guided search, but this approach has several drawbacks:
...
### Enabling the statement index

The semantic-guided retriever requires the statement vector index. While the default configuration currently includes the statement index, this may change in a future release. To ensure the statement index is always available, explicitly specify it when creating your vector store:
...
<Aside type="caution">
  The statement index is approximately 10x larger than the chunk index. Enabling it will significantly increase storage and embedding costs.
</Aside>
...
from graphrag_toolkit.lexical_graph.storage import GraphStoreFactory
from graphrag_toolkit.lexical_graph.storage import VectorStoreFactory
from graphrag_toolkit.lexical_graph.retrieval.retrievers.deprecated import RerankingBeamGraphSearch, StatementCosineSimilaritySearch, KeywordRankingSearch
