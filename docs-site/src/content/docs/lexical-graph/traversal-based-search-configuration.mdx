...
The `EntityNetworkSearch` retriever offers a good balance of local and global searches, and may suffice for many queries: experiment with using the `EntityNetworkSearch` by itself to determine whether it meets your query needs.
...
When using the `ChunkBasedSearch` and `EntityNetworkSearch` retrievers, `vss_top_k` affects the number of chunk-based starting points for each retrieval operation. Experiment with reducing `vss_top_k` to determine the optimal balance between the number of starting points (which in turn determine the number of backend graph queries), query latency and accuracy.
...
When using the `EntityBasedSearch` and `EntityNetworkSearch` retrievers, `ec_max_contexts` affects the number of starting points for each retrieval operation. Experiment with reducing `ec_max_contexts` to determine the optimal balance between the number of starting points (which in turn determine the number of backend graph queries), query latency and accuracy.
