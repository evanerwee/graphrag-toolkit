...
"""CSV Schema Provider Module for Document Graph Operations.

This module provides a schema provider for CSV (Comma-Separated Values) files.
It discovers and generates ETL schemas from CSV files for processing and loading
the data into a document graph.

The module includes the following components:
- CSVSchemaProvider: Schema provider for CSV files

The CSV schema provider uses the CSVSchemaDiscoveryProvider to analyze CSV files
and generate appropriate ETL schemas. It supports loading schemas from CSV files
and optionally saving the discovered schemas to JSON files.

Usage:
    # Get a CSV schema provider for a specific file
    from document_graph.schema.providers.schema_provider_factory import get_schema_provider
    
    config = {
        "provider_type": "csv",
        "schema_id": "my_csv_schema",
        "connection_config": {
...
class CSVSchemaProvider(SchemaProviderBase):
    """
    Schema provider for CSV (Comma-Separated Values) files.
    
    This class provides functionality to discover and generate ETL schemas from CSV files.
    It uses the CSVSchemaDiscoveryProvider to analyze CSV files and extract field information
    to create appropriate ETL schemas for processing and loading the data into a document graph.
    
    The provider supports loading schemas directly from CSV files and optionally saving
    the discovered schemas to JSON files for later use.
    
    Attributes:
        config (SchemaProviderConfig): Configuration for the provider.
        csv_path (Path): The path to the CSV file to analyze.
    """

    def __init__(self, config: SchemaProviderConfig):
        """
        Initialize a CSV schema provider with the given configuration.
        
        Args:
            config (SchemaProviderConfig): Configuration for the provider, including
                the path to the CSV file in the connection_config.
    ...
    @classmethod
    def from_config(cls, config: SchemaProviderConfig) -> "CSVSchemaProvider":
        """
        Factory method to construct a CSV provider from a configuration.
        
        Args:
            config (SchemaProviderConfig): Configuration for the provider.
            
        Returns:
            CSVSchemaProvider: A new instance of the CSV schema provider.
        """
        return cls(config)

    def load_schema(self, source=None, **kwargs) -> ETLSchema:
        """
        Load (or generate) an ETL schema from a CSV file using schema discovery.
        
        This method uses the CSVSchemaDiscoveryProvider to analyze the CSV file and
        ...
        Returns:
            ETLSchema: A complete ETL schema object that describes the structure
                of the CSV file and how it should be processed.
                
        Raises:
            FileNotFoundError: If the CSV file does not exist.
            ValueError: If the CSV file cannot be parsed or has no discoverable fields.
        """
        discovery = CSVSchemaDiscoveryProvider(source=self.csv_path)
    ...
    def save_schema(self, output_path: Path) -> None:
        ...
        This method discovers the schema from the CSV file and saves it to a JSON file
        ...
        try:
            ...
            logger.info(f"Saved CSV schema to file: {output_path}")
    ...
    def get_schema_id(self) -> str:
        ...
        configuration or generated from the CSV file name.
        
        Returns:
            str: A unique identifier for the schema.
        """
        return self.config.schema_id or f"csv-schema-{self.csv_path.stem}"
