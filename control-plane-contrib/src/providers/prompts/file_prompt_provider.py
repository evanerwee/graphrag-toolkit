...
class FilePromptProvider(PromptProvider):
    ...
    def __init__(self, config: FilePromptProviderConfig, system_prompt_file: str = "system_prompt.txt", user_prompt_file: str = "user_prompt.txt"):
        ...
        logger.info("[Prompt Debug] Initialized FilePromptProvider")
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                ...
                is_json_format = (
                        ...
                        or (self.config.format is None and filename.lower().endswith(".json"))
                )

                return json.loads(content) if is_json_format else content.strip()
